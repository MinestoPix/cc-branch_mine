...
init_settings = function()
    ...
    settings["trunk_width"] = settings["trunk_width"] or 3
    settings["trunk_height"] = settings["trunk_height"] or 3

    -- turtle(slots) config
    -- chest and torch slots are to keep track of supplies
    settings["torch_slot"] = settings["torch_slot"] or 14
    ----settings["chest_slot"] = settings["chest_slot"] or 2
  settings["fuel_slot"] = settings["fuel_slot"] or 14
  settings["torch_chest_slot"] = settings["torch_chest_slot"] or 15
  settings["coal_chest_slot"] = settings["coal_chest_slot"] or 16
    -- cobblestone slot use to place so torch can be placed
    settings["cobblestone_slot"] = settings["cobblestone_slot"] or 1

    -- level of coal to reach when refuelling
    ...
    -- do broadcast
    -- on receiver will determine weather or not to retransmit (range extension)
    ...
    --  side of modem if peripheral.find() is picking the wired one
    ...
    --  side monitor if peripheral.find() is picking an undesired one (works with networked monitors)
...
init_progress = function()
    ...
    if (progress == nil) then -- CHANGE IT ALL :(
        progress = {}
        progress["task"] = nil
    ...
    if (settings["debug"]) then
      ...
      if(turtle == nil)then
        ...
        logged_from = "[turt]"
            ...
            handle.writeLine("["..tostring(os.time()).."]"..logged_from..": "..tostring(message))
            handle.close()
        end
    end
end
...
local function send_message(message_type, message_data)
    ...
    if (message_data["type"] == nil) or (message_data["turtle_id"] == nil) then
        ...
        print_error(message_error_failed_to_send_message.." - no type or id given", false, false)
        return
    end
    temp_seralized = string.gsub(textutils.serialize(message_data), "[\n ]", "")
    log("send: "..temp_seralized)
    ...
    if (message_type == "check") or (message_type == "branch_update") then
        ...
        while (do_loop) do
            ...
            if (event == "modem_message") then
                ...
                -- confirm event
                if (message_data["type"] == "confirm") and
...
-- used by receiver to confirm request
local function send_confirm(id)
    local confirm_data = {}
    confirm_data["type"] = "confirm"
    confirm_data["turtle_id"] = id
    if not (progress["retransmit_id"] == nil) then
        confirm_data["retransmit_id"] = progress["retransmit_id"]
    end
    temp_seralized = string.gsub(textutils.serialize(confirm_data), "[\n ]", "")
    log("confirm: "..temp_seralized)
    transmitter.transmit(settings["receive_channel"], settings["transmit_channel"], textutils.serialize(confirm_data))
end
...
-- incrementing = rotating counter-clockwise
...
local function chest_get(slot, toSlot)
  ...
  while not (turtle.suck()) do
    term.setCursorPos(1,ids_line+3)
        color_write(message_error_fuel, colors.cyan)
        sleep(10)
        term.setCursorPos(1,ids_line+3)
        clear_line()
  end
...
-- test if it is necessary to get resources
local function get_fuel_and_supplies_if_needed(required_fuel)
    ...
    if (turtle.getFuelLevel() < required_fuel) then
        ...
        while (need_fuel) do
            chest_get(settings["coal_chest_slot"],settings["fuel_slot"])
            use_all_fuel()
            need_fuel = (turtle.getFuelLevel() < required_fuel)
        end
    end
    ...
    if (--[[need_chests or ]]need_torches) then
        ...
        while (need_torches) do
            ...
            ----turtle.drop()
            ----turtle.suck()
      chest_get(settings["torch_chest_slot"],settings["torch_slot"])
...
local function get_directions()
  ...
  transmitter.open(settings["transmit_channel"])
        ...
        log("received message")
                ...
                -- waiting for directions from main
                if (message_data["type"] == "initiate") and
                  ((not (turtle == nil) and (message_data["turtle_id"] == os.computerID())) or
                   ((turtle == nil) and (message_data["turtle_id"] == progress["paired_id"]) and (message_data["retransmit_id"] == os.computerID()))) then
                    do_loop = false
          log("success")
          directions = message_data["directions"]
                end
            elseif (event == "timer") then
                log("timer")
        send_message("initiate")
...
local function get_initiate()
  ...
  if(progress["branch_progress"]["left"] <= progress["branch_progress"]["right"])then
    initiate["side"] = "left"
    initiate["length"] = progress["branch_progress"]["left"]
  else
    initiate["side"] = "right"
    initiate["length"] = progress["branch_progress"]["right"]
  end
  log(textutils.serialize(initiate))
  return initiate
end
...
-- main functions
empty_inventory = function() -- CHANGE THIS --
    for i=1,16 do
        set_task("Emptying", string.format("%3d%%", (i/16)*100))
        turtle.select(i)
        if (i == settings["torch_slot"]) or (i == settings["cobblestone_slot"])  or (i == settings["torch_chest_slot"])  or (i == settings["coal_chest_slot"]) then
        else
            is_test_block = false
...
local function dig_branch()
    ...
    -- each path through loop is one half of branch (each side of trunk) -- CHANGE THIS -- each side is separate /
    for x=progress["branch"]["side"],2 do --                                                              <-- remove this for
        ...
        if (progress["branch"]["height"] == 1) then
            ...
            for i=progress["branch"]["progress"],settings["branch_length"] do
                ...
                -- verify blocks are in place for torches (placed later)
...
local function run_turtle_main()
    ...
    term.setCursorPos(1,ids_line+4)
        color_write("Put coal enderchest in slot "..settings["coal_chest_slot"]..", ", colors.cyan)
    term.setCursorPos(1,ids_line+5)
        color_write("and torch enderchest in slot "..settings["torch_chest_slot"], colors.cyan)
        term.setCursorPos(1,ids_line+6)
        color_write("Put any \"do not mine\" blocks in others", colors.cyan)
        term.setCursorPos(3,ids_line+7)
        color_write("i.e.: cobblestone, stone,", colors.cyan)
        term.setCursorPos(4,ids_line+8)
        ...
        clear_line()
    term.setCursorPos(1,ids_line+8)
        clear_line()
    end

    for i=1,16 do
    if not(i==settings["torch_slot"] or i==settings["fuel_slot"] or i==settings["torch_chest_slot"] or i==settings["coal_chest_slot"] or i==settings["cobblestone_slot"] ) then
  ...
  -- verify there is at least __some__ fuel to get started
  ...
  local start_directions = get_directions()
    ...
    -- digs out the room if has to, gets to right position
    turtle_start(start_directions)
    ...
    elseif (receiver_data["type"] == "initiate") and (progress["paired_id"] == receiver_data["turtle_id"]) and (progress["retransmit_id"] == receiver_data["retransmit_id"]) then
      ...
      send_message("initiate",get_initiate())
        ...
        elseif (receiver_data["type"] == "branch_update") and (progress["paired_id"] == receiver_data["turtle_id"]) and (progress["retransmit_id"] == receiver_data["retransmit_id"]) then
            ...
            term.setCursorPos(unpack(current_branch_location))
            color_write(string.format("%3d", progress["branch"]["current"]), colors.yellow)
            send_confirm(receiver_data["turtle_id"])
            retransmit = true
        -- task event
